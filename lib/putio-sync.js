...
var PutIOSync = function (opts) {
    ...
    // Ensure monogamy
    try {
        var pid = npid.create(path.join(projectBase, "sync.pid"));
        pid.removeOnExit();
     ...
     * @param downloadedCallback callbackFunction called when file is downloaded
     */
    var enqueueDownload = function (sync, fileData, target, downloadedCallback) {
        downloadQueue.push({
            sync: sync,
            fileData: fileData,
            target: target,
    ...
    downloadQueueEvents.on("next", function (slot) {
        ...
        downloadFile(toDownload.sync, toDownload.fileData, toDownload.target, slot, toDownload.number, toDownload.downloadedCallback);
    });
     ...
     * @param downloadedCallback callbackFunction called when file is downloaded
     */
    var downloadFile = function (sync, fileData, target, slot, number, downloadedCallback) {
                            ...
                            fs.rename(tempFilename, target, function () {
                                ...
                                if (sync.delete) {
                                    ...
                                    api.files.delete(fileData.id, function (delData) {
                                        ...
                                        else if (downloadedCallback !== null)
                                            downloadedCallback();
                                    })
                                } else if (downloadedCallback !== null)
                                    downloadedCallback();
     ...
     * @param finishCallback Callback function for when the syncing finished
     */
    var syncFolderRecursive = function (sync, folderId, localPath, depth, finishCallback) {
        api.files.list(folderId, function (data) {
            ...
            var filesToSync = data.files.length;
            ...
            var incFileSynced = function() {
                filesSynced++;
                if (filesSynced == filesToSync) {
                    finishCallback();
                }
            }
            ...
            _.each(data.files, function (file) {
                ...
                if (file.content_type === "application/x-directory") {
                    ...
                    syncFolderRecursive(sync, file.id, localFile, depth + 1, incFileSynced);
                } else {
                        ...
                        if (exists) {
                            ...
                            fs.stat(localFile, function (err, stats) {
                                if (stats.size === file.size) {
                                    ...
                                    enqueueDownload(sync, file, localFile, incFileSynced);
                                }
                            });
                        } else {
     ...
     * @param finishCallback Callback function for when the syncing finished
     */
    var syncFolder = function (sync, finishCallback) {
        ...
        apiHelper.findFolderId(sync.remote_path, function (id) {
            ...
            syncFolderRecursive(sync, id, sync.local_path, 0, finishCallback);
    ...
    this.startSync = function () {
        ...
        var foldersToSync = config.sync.length;
        var foldersSynced = 0;
        ...
        _.each(config.sync, function (sync) {
            ...
            fs.exists(sync.local_path, function (exists) {
                if (exists) {
                    syncFolder(sync, function() {
                        foldersSynced++;
                        if (foldersSynced === foldersToSync) {
                            logv(1, ("Sync ended successfully").green);
                            waitForNextExecution();
                        }
                    });
                } else {
